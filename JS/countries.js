...
function createCountList(){
  const numCountries = country_list.length;

  let i = 0, ul_list_id;

  country_list.forEach((country,index) => {
    if(index % Math.ceil(numCountries/num_ul_lists) == 0){
      ul_list_id = `list-${i}`;
      countryList.innerHTML += `<ul id=${ul_list_id}></ul>`;
      i++;
    }
...
// // country filter
// // input event listener fires when input value changes
...
//  reset country list after search
